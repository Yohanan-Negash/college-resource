...
const storyContent = {
    title: "The Story Behind CollegeResource",
    paragraphs: [
        ...
        "That's why we created CollegeResource—a platform born from real experiences and designed to bridge the gap between academic achievement and industry success."
    ]
};
        ...
        <div className="bg-[#2A2D3E]/50 backdrop-blur-sm p-8 rounded-xl border border-zinc-700/50 mb-8">
            ...
            <div className="space-y-4 text-lg text-zinc-300">
                {storyContent.paragraphs.map((paragraph, index) => (
                    <p key={index} className={index === storyContent.paragraphs.length - 1 ? "font-medium text-white" : ""}>
                        {paragraph}
                    </p>
                ))}
            </div>
